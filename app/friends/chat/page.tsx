...
const ChatPage = () => {
  ...
  const fetchInstructors = useCallback(async () => {
    try {
      const res = await fetch("/api/chat/users/mutuals");
      if (res.ok) {
        const data = await res.json();
        setInstructors(data);
      }
      ...
      setToast({ show: true, message: "Gagal memuat daftar teman", type: "error" });
    }
  }, []);
  ...
  const fetchFavorites = useCallback(async () => {
    try {
      ...
      setToast({ show: true, message: "Gagal memuat teman favorit", type: "error" });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const instructorId = searchParams.get("userId");
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (unreadMessageIds.length > 0) {
          fetch("/api/chat/users/messages/read", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                    ...
                    >
                      <div className="relative shrink-0">
                        <Avatar className="h-12 w-12 border-2 border-slate-100">
                          <AvatarImage src={conv.otherUser.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${conv.otherUser.name}`} />
                          <AvatarFallback className="bg-teal-500 text-white font-black text-lg">
                            {conv.otherUser.name?.charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        {isParticipantOnline(conv.otherUser.id) && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-1">
                          <p className="font-bold text-slate-800 truncate">{conv.otherUser.name}</p>
                          {conv.lastMessage && (
              ...
              {selectedConversation ? (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="relative">
                        <Avatar className="h-10 w-10 border-2 border-white shadow-sm ring-2 ring-slate-100">
                          <AvatarImage src={selectedConversation.otherUser.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${selectedConversation.otherUser.name}`} />
                          <AvatarFallback className="bg-teal-500 text-white font-black text-lg">
                            {selectedConversation.otherUser.name?.charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        {isParticipantOnline(selectedConversation.otherUser.id) && (
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-500 flex items-center gap-1">
                          {isParticipantOnline(selectedConversation.otherUser.id) ? (
                            ...
                            lastSeenMap.get(selectedConversation.otherUser.id) ? 
                            `Last seen ${formatRelativeTime(lastSeenMap.get(selectedConversation.otherUser.id)!)}` : "Offline"
                          )}
                ...
                <div className="flex flex-col items-center justify-center h-full p-8 text-center relative">
                  ...
                  <p className="text-slate-500 font-medium max-w-xs">
                    Pilih percakapan dari daftar atau mulai chat baru dengan temanmu di IRMA VERSE.
                  </p>
                  <button
                    onClick={() => setShowNewChatModal(true)}
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-md max-h-[85dvh] flex flex-col overflow-hidden border-4 border-emerald-500 shadow-[10px_10px_0_0_#065f46] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b-4 border-slate-100 bg-emerald-50 flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="text-xs font-bold text-emerald-600 mt-0.5">Pilih teman untuk mengobrol</p>
              </div>
              <button 
